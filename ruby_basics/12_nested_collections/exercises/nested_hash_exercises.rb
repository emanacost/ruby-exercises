def find_language_information(languages, language_name)
  ...
  # }
  result = languages.find { |k, v| k == language_name }
  result[1]
end
...
def add_language(languages, language_name, language_info_value)
  ...
  lang_insert = { language_name => language_info_value}
  languages.update(lang_insert)
  languages
end
...
def find_beautiful_languages(languages)
  ...
  languages.select { |name, value| name if value[:is_beautiful?] == true}
end
